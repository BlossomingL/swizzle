...
def visualize_dense_schedule(
    ...
    show_round_text=True, max_text_cells=None
):
    """
    ...
    for w in range(0, total_batches):
        ...
        # 绘图
        max_round_digits = len(str(int(rounds_mat.max()))) if rounds_mat.size else 1
        cell_px = max(56, max_round_digits * 16 + 18)
        dpi = 200
        width_in = max(4.0, n * cell_px / dpi)
        height_in = max(4.0, m * cell_px / dpi)
        fig, ax = plt.subplots(figsize=(width_in, height_in), dpi=dpi)

        im = ax.imshow(core_mat, origin='upper', cmap=cmap, vmin=0, vmax=k-1,
        ...
        draw_round_text = bool(show_round_text)
        ...
        if draw_round_text:
            ...
            for i in range(m):
                for j in range(n):
                    r_id = rounds_mat[i, j]
                    if r_id >= 0:
                        ax.text(j, i, str(r_id), ha='center', va='center',
                                fontsize=text_size, color='white', fontfamily='Consolas')
        ...
        output_path = output_dir / f'Dense_TND_k{k}_N1{N1}_mode{mode_chosen}_batch{w}_m{m}_n{n}.png'
...
visualize_dense_schedule(k, list_m, list_n, N1, prefix, 2)
